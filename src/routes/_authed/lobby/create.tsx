...
const CATEGORIES = ['Object', 'Nature', 'Random', 'Person', 'Action', 'World'] as const
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1">Configure your game settings</p>
        </div>
        ...
        >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Set up how players are divided into teams</CardDescription>
            </CardHeader>
              ...
              <form.Field name="teamAssignment">
                {(field) => (
                  <Field>
                    ...
                    >
                      {[
                        { value: 'random', label: 'Random', description: 'Players are shuffled into teams automatically' },
                        { value: 'host', label: 'Host assigns', description: 'Host places each player into a team' },
                        { value: 'self', label: 'Self-select', description: 'Players choose their own team' },
                      ].map((opt) => (
                        <div key={opt.value} className="flex items-start gap-2">
                          <RadioGroupItem value={opt.value} id={`assign-${opt.value}`} className="mt-0.5" />
                          <Label htmlFor={`assign-${opt.value}`} className="font-normal cursor-pointer">
                ...
                {(field) => (
                  <Field>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium tabular-nums">{field.state.value}s</span>
                    </div>
                    <Slider
                      value={[field.state.value]}
                      ...
                      <Field>
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <Label htmlFor="skip-back" className="font-normal cursor-pointer">Return to deck</Label>
                          </div>
                          <div className="flex items-center gap-2">
                            <RadioGroupItem value="discard" id="skip-discard" />
                            <Label htmlFor="skip-discard" className="font-normal cursor-pointer">Discard</Label>
                          </div>
                        </RadioGroup>
                      </Field>
                    )}
              ...
              <form.Field name="winCondition">
                {(field) => (
                  <Field>
                    ...
                    >
                      {[
                        { value: 'board', label: 'Classic board', description: 'First team to reach the end of the board wins' },
                        { value: 'rounds', label: 'Fixed rounds', description: 'Most points after a set number of rounds' },
                      ].map((opt) => (
                        <div key={opt.value} className="flex items-start gap-2">
                          <RadioGroupItem value={opt.value} id={`win-${opt.value}`} className="mt-0.5" />
                          <Label htmlFor={`win-${opt.value}`} className="font-normal cursor-pointer">
                            <span className="font-medium">{opt.label}</span>
                            <span className="text-muted-foreground"> — {opt.description}</span>
                          </Label>
                        </div>
                        ...
                        {(field) => (
                          <Field>
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-medium tabular-nums">{field.state.value} spaces</span>
                            </div>
                            <Slider
                              value={[field.state.value]}
                              onValueChange={(values) => field.handleChange(values[0])}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose which categories to include and add your own words</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">

              <Field>
                <FieldLabel>Categories</FieldLabel>
                <FieldDescription>At least one must be enabled</FieldDescription>
              ...
              <form.Field name="allowDuplicates">
                {(field) => (
                  <div className="flex items-center justify-between rounded-lg border px-3 py-3">
                    <div>
                      <Label htmlFor="allow-duplicates" className="cursor-pointer">Allow duplicate cards</Label>
              ...
              <form.Field name="customWords">
                {(field) => (
                  <Field>
                    ...
                    <FieldDescription>
                      One word or phrase per line — added to the deck alongside standard cards
                    </FieldDescription>
                    <Textarea
